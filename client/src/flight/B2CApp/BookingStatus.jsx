    ...
    if(apiCalled === false){
      ...
      const fatchData = async () =>{
        const response = await axios.post(
          `http://localhost:8001/flight/bookingstatus`, ({transactionId})
        );
  ...
  return (
    <div>
      <div className=' w-[70%] m-auto pt-3'>
        <div className='  border-b-2 border-solid border-gray-950'>
          <p className=' items-baseline'>E-Ticket</p>
          {/* <p><span className=' text-green-600 float-right text-4xl'>A_Raj</span></p> */}
        </div>
        {
          itineraries.map((itinerary, index)=>{
            ...
            // for bookingDate and timme
            const bookDate = general.bookDate;
            const bookingDate = new Date(bookDate)
            const bookingLocalTime = timeFormat.format(bookingDate);

            
            return(
              <div  key={index}>
                <div className=' pb-3 pt-3 border-b-2 border-solid border-gray-950'>
                  <p>Payment Booking ID : <span className=' text-green-600'>{general.queuingOfficeId}</span></p>
                  <p>Booked on : <span>{formatDate(bookDate)}, {bookingLocalTime}</span></p>
                </div>
          ...
          <div className=' w-full pl-7'>
            ...
            </div>
            {
              travelers.map((traveler, index)=>{
                return(
                  <p key={index} className=' pt-3 border-b border-solid border-gray-400 text-sm font-medium'>{traveler.firstName.toUpperCase() + " "+ traveler.lastName.toUpperCase()} <span className=' float-right'>{general.pnr}</span></p>
                )
              })
            }
          </div>
